...
Проект настроен для автоматического развертывания на [Railway.app](https://railway.app).

### Быстрое развертывание:

1. **Создайте проект на Railway** и подключите репозиторий
2. **Добавьте PostgreSQL базу данных** через Railway Dashboard
3. **Настройте переменные окружения:**
   - `JWT_SECRET`: секретный ключ (минимум 32 символа)
   - `JWT_EXPIRATION_SECONDS`: 3600
   - `UPLOADS_DIR`: uploads
4. **Railway автоматически создаст два сервиса** (backend + frontend)

Подробная инструкция: [RAILWAY_DEPLOYMENT.md](RAILWAY_DEPLOYMENT.md)

Переменные окружения: [RAILWAY_ENV_VARS.md](RAILWAY_ENV_VARS.md)
