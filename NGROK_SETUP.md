# 🚀 Запуск с ngrok (для учебных целей)

## 📋 Предварительные требования

1. **PostgreSQL** должен быть запущен локально
2. **ngrok** должен быть установлен
3. **Java 17+** должен быть установлен

## ⚡ Быстрый запуск

### Вариант 1: Интерактивный выбор (рекомендуется)
```bash
# Выберите нужную конфигурацию из меню:
run-with-ngrok-interactive.bat
```

### Вариант 2: Backend Public + Frontend Local (ваш запрос)
```bash
# Backend доступен из интернета, frontend локальный:
run-backend-public-frontend-local.bat
```

### Вариант 3: Полностью публичное приложение
```bash
# И backend И frontend доступны из интернета:
run-fully-public.bat
```

### Вариант 3: Простой скрипт (обе службы публичные)
```bash
# Дважды кликните на run-with-ngrok.bat
# Или запустите в командной строке:
run-with-ngrok.bat
```

### Вариант 2: Ручной запуск

#### Шаг 1: Запустите backend
```bash
cd backend
java -jar target/zoopark-backend-0.0.1-SNAPSHOT.jar
```

#### Шаг 2: Запустите ngrok (в новом терминале)
```bash
ngrok http 8080
```

#### Шаг 3: Скопируйте публичный URL
```
Forwarding    https://abc123.ngrok.io -> http://localhost:8080
```

## 🌐 Доступ к приложению

### После запуска скрипта:

1. **Скопируйте ngrok URL** из терминала ngrok:
   ```
   Forwarding    https://abc123.ngrok.io -> http://localhost:8080
   ```

2. **Настройте frontend для работы с backend:**
   ```bash
   # Откройте новый терминал и выполните:
   cd frontend
   echo "VITE_API_BASE_URL=https://abc123.ngrok.io" > .env.local
   ```

3. **ИЛИ настройте переменную окружения:**
   ```bash
   # В терминале frontend выполните:
   set VITE_API_BASE_URL=https://abc123.ngrok.io
   # И перезапустите npm run dev
   ```

### Доступ:
- **Frontend (сайт):** `http://localhost:5173`
- **Backend API:** `https://abc123.ngrok.io/api/health`
- **ngrok Dashboard:** `http://localhost:4040`

## 🛠️ Установка ngrok

### Windows (через Chocolatey):
```bash
choco install ngrok
```

### Ручная установка:
1. Скачайте с [ngrok.com/download](https://ngrok.com/download)
2. Распакуйте в папку из PATH
3. Авторизуйтесь: `ngrok config add-authtoken YOUR_TOKEN`

## 📝 Локальная разработка

Для обычной разработки без ngrok:
```bash
# Backend
cd backend
mvn spring-boot:run

# Frontend (в новом терминале)
cd frontend
npm run dev
```

## ⚠️ Важно

- **ngrok URL меняется** при каждом перезапуске
- **Бесплатный план** имеет ограничения по трафику
- **Для production** используйте Railway или Heroku

## 🔧 Troubleshooting

### ERR_NGROK_334: Endpoint already online
```
ERROR: failed to start tunnel: The endpoint is already online
```
**Решение:**
1. **Запустите супер-очистку:** `stop-ngrok.bat` (агрессивная остановка всех процессов)
2. **ИЛИ используйте комбо-скрипт:** `cleanup-and-run.bat` (очистка + автоматический запуск)
3. **Если не помогает:** Перезагрузите компьютер
4. **Профилактика:** Всегда запускайте `stop-ngrok.bat` перед новыми запусками

### Что делать если проблема повторяется:
1. **Проверьте Диспетчер задач** (Ctrl+Shift+Esc)
2. **Найдите процессы:** ngrok.exe, java.exe, node.exe
3. **Принудительно завершите** их все
4. **Запустите** `stop-ngrok.bat` для гарантии
5. **Альтернатива:** Используйте Railway вместо ngrok для production

### Vite blocks ngrok hosts
```
Blocked request. This host ("xyz.ngrok.io") is not allowed.
```
**Решение:** Vite блокирует ngrok хосты по умолчанию. Уже исправлено в `vite.config.js`.

### Ручной способ (если скрипты не работают):
```bash
# В одном терминале - остановка процессов:
stop-ngrok.bat

# Подождите 10 секунд...

# В другом терминале - запуск по частям:
cd backend
java -jar target/zoopark-backend-0.0.1-SNAPSHOT.jar

# Подождите пока backend запустится...

# Новый терминал для ngrok:
ngrok http 8080

# Скопируйте URL и настройте frontend...
```

### ERR_NGROK_313: Custom subdomains require paid plan
```
ERROR: Only paid plans may create endpoints with custom subdomains
```
**Решение:**
- Скрипты автоматически используют бесплатные случайные URL
- Каждый запуск создает новые URL (например: `https://abc123.ngrok.io`)
- Для постоянных URL нужен платный план ngrok

### Backend не запускается:
```bash
# Проверьте PostgreSQL
Get-Service postgresql*

# Проверьте порт 8080
netstat -ano | findstr :8080
```

### ngrok не работает:
```bash
# Проверьте установку
ngrok version

# Авторизуйтесь
ngrok config add-authtoken YOUR_TOKEN
```
